...
public class PublicApiController {
	...
	private String baseDate = "20240408";
    ...
    public void insert() throws Exception {
		...
		for (int j = 0; j < parse_item.size(); j++) {
			...
			JSONObject tmp = (JSONObject) parse_item.get(j);
			...
			System.out.println(sw_category);
			System.out.println(sw_fcstValue);
			System.out.println(sw_locx + ", " + sw_locy);
			System.out.println(sw_date);
	...
	public void dustInsert() throws Exception {
        ...
        // 2. 오픈 API의 요청 규격에 맞는 파라미터 생성. serviceKey 부분에 반드시 내가 인증받은 인코딩 키를 넣어주어야 한다!!
        urlBuilder.append("?" + URLEncoder.encode("serviceKey","UTF-8") + "="+serviceKey); /*Service Key*/
        urlBuilder.append("&" + URLEncoder.encode("returnType","UTF-8") + "=" + URLEncoder.encode("json", "UTF-8")); /*페이지번호*/
        urlBuilder.append("&" + URLEncoder.encode("numOfRows","UTF-8") + "=" + URLEncoder.encode("10", "UTF-8")); /*한 페이지 결과 수*/
        urlBuilder.append("&" + URLEncoder.encode("pageNo","UTF-8") + "=" + URLEncoder.encode("1", "UTF-8")); /*요청자료형식(XML/JSON)*/
        urlBuilder.append("&" + URLEncoder.encode("searchDate","UTF-8") + "=" + URLEncoder.encode(formattedDate, "UTF-8")); /*지역코드가 11B00000 인 코드를 구해옴*/
        urlBuilder.append("&" + URLEncoder.encode("informCode","UTF-8") + "=" + URLEncoder.encode(informCode, "UTF-8")); /*3/21*/
...
//        System.out.println(body);
		...
		for(int i=0;i<=2;i++){
			...
			JSONObject dustArray = (JSONObject)items.get(i);
			...
			System.out.println("informData : "+informData);
...
//		System.out.println("Response code: " + conn.getResponseCode());
       ...
       // System.out.println("전체 response = " + result);
	    ...
	    //System.out.println("Response code: " + conn01.getResponseCode());
       ...
       // System.out.println("전체 response111 = " + result01);
       ...
       // System.out.println("전체 response222 = " + result02);
        ...
        //System.out.println("전체 response333 = " + result03);
